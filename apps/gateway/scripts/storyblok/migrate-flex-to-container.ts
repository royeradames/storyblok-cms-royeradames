#!/usr/bin/env bun
/**
 * Migrate Flex → Container: change component name in all stories.
 *
 * After renaming shadcn_flex to shadcn_container, existing story content
 * still has component: "shared_shadcn_flex". This script walks all stories,
 * finds any block with that component, changes it to "shared_shadcn_container",
 * and saves the story.
 *
 * Loads STORYBLOK_SPACE_ID and STORYBLOK_PERSONAL_ACCESS_TOKEN from apps/gateway/.env
 *
 * Usage (from apps/gateway):
 *   bun run storyblok:migrate-flex-to-container
 */
...
const CONTAINER_COMPONENT = "shared_shadcn_container";
const DELAY_MS = 400;
 ...
 * set component = CONTAINER_COMPONENT. Returns true if any change was made.
 */
function migrateInPlace(obj: unknown): boolean {
  ...
  if (o.component === FLEX_COMPONENT) {
    o.component = CONTAINER_COMPONENT;
    changed = true;
  }
...
async function main() {
  console.log("🚀 Migrate Flex → Container (shared_shadcn_flex → shared_shadcn_container)\n");
